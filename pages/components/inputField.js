...
export default function InputField({ setFuncErr, setFunc, value, err, placeholder, type, filled }) {
    function handle(e) {
        const newValue = e.target.value;
        if (type === 'name') {
            setFunc({ ...value, name: newValue });
            setFuncErr({ ...err, nameErr: nameValidate(newValue) });
        } else if (type === 'id') {
            setFunc({ ...value, id: newValue });
            setFuncErr({ ...err, idErr: idValidate(newValue) });
        } else if (type === 'phone') {
            setFunc({ ...value, phone: newValue });
            setFuncErr({ ...err, phoneErr: phoneValidate(newValue) });
        } else if (type === 'email') {
            setFunc({ ...value, email: newValue });
            setFuncErr({ ...err, emailErr: emailValidate(newValue) });
        }
        filled();
    }
    return (
