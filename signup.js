...
export default function signup() {
    ...
    const handlePswChange = (e) => {
        ...
        if (newValue.length < 6 || newValue.length > 16) {
            ...
            if (newValue == confirm && confirm != "") {
                setPswCongirmErr(false);
            } else {
                setPswCongirmErr(true);
            }
        }
        filled();
    };
    ...
    const [pswConfirmErr, setPswCongirmErr] = useState(false);
    ...
    const filled = () => {
        ...
        if (!nameErr && !idErr && !phoneErr && !emailErr && !pswErr && name != "" && id != "" && phone != "" && email != "" && psw != "" && psw == pswConfirm) {
            setIsSubmit(true);
            console.log("123");
        }
    }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const name = nameInput.current.value;
        const birthday = yearInput.current.value + "-" + monthInput.current.value + "-" + dayInput.current.value;
        const id = idInput.current.value;
        const phone = phoneInput.current.value;
        const email = emailInput.current.value;
        ...
        if (isSubmit) {
            console.log(
                JSON.stringify({ Data: { name, birthday, id, phone, email, psw }, source_id: 4 })
            );
        }
    ...
    return (
        <div className="wid100 fx fx_center signup">
            <form className="wid50" onSubmit={handleSubmit}>
                <label className="wid100 fx fx_center" style={{ margin: "20px 0", fontSize: "1.3em", fontWeight: "bold" }}>會員註冊</label>

                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>姓名<label>*</label></p></div>
                    <InputField
                ...
                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>身分證字號<label>*</label></p></div>
                    <InputField
                        setFuncErr={setIdErr}
                        setFunc={setId}
                        value={id}
                ...
                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>出生年月日<label>*</label></p></div>
                    <div className="fx fx_nowrap fx_center fx_between">
                        <select className="wid40" onChange={handleYearChange} value={year} ref={yearInput}>
                            {createYear()}
                        </select>
                        <span className={styles.formSelectSpan}>年</span>
                        <select className="wid30" onChange={handleMonthChange} value={month} ref={monthInput}>
                            {createMonth()}
                        </select>
                        <span className={styles.formSelectSpan}>月</span>
                        <select className="wid30" onChange={handleDayChange} value={day} ref={dayInput}>
                ...
                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>行動電話<label>*</label></p></div>
                    <InputField
                ...
                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>Email<label>*</label></p></div>
                    <InputField
                ...
                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>密碼<label>*</label></p></div>
                    <InputBase fullWidth error required
                ...
                <div className={styles.formGroup}>
                    <div className={styles.formTitle}><p>確認密碼<label>*</label></p></div>
                    <InputBase fullWidth error required
                        type={values.showConfirmPassword ? 'text' : 'password'}
                        value={pswConfirm}
                        onChange={handlePswConfirmChange}
                        inputRef={pswConfirmInput}
                        inputProps={{ 'aria-label': 'naked' }}
                ...
                <div className={styles.btnGroup + " fx fx_nowrap"}>
                    <button type="reset" className="btn btn50 btn_01">重新填寫</button>
                    <div style={{ width: "5%" }}></div>
                    <button type="submit" className={isSubmit ? "btn btn50 btn_00" : "btn btn50 btn_disable"}>確認送出</button>
                </div>
